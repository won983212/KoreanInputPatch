	...
	public void drawIndicator(int x, int y, int len, int maxLen, String unit) {
		if(input.isComponentFocused()) {
			...
			if(maxLen == len) {
				...
				alertBg = 0xfffb8c00;
				alertText = maxLen - len + unit + " 남음";
			}

			String idiText = (Korean2Input.isKorMode() ? "한" : "영");
			int textWidth = fr.getStringWidth(idiText);
			
			final String _alertText = alertText;
			final int _alertBg = alertBg;
			KoreanInputPatch.instance.getEventHandler().addTopRenderQueue(new Runnable() {
				...
				public void run() {
					...
					if(_alertText != null) {
						...
						Gui.drawRect(alertX, y, alertX + fr.getStringWidth(_alertText) + 4, y + height, _alertBg);
						fr.drawString(_alertText, alertX + 2, y + 1, 0xffffffff);
					}
